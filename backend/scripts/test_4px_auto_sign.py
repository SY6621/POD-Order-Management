"""
4PX API 测试 - 自动生成签名（使用HTML工具的JSON格式）
"""
import hashlib
import json
import time
...
def create_order():
    ...
    # 替换ref_no（保持其他参数不变）
    # 使用replace而不是format，避免JSON中的{}与format冲突
    biz_content = HTML_JSON_TEMPLATE.replace("{ref_no}", ref_no)
