...
- [backend/scripts/add_service_link_fields.sql](file://backend/scripts/add_service_link_fields.sql)
- [backend/scripts/create_service_link_logs_table.sql](file://backend/scripts/create_service_link_logs_table.sql)
- [backend/scripts/setup_service_link.py](file://backend/scripts/setup_service_link.py)
- [backend/scripts/fetch_today_orders.py](file://backend/scripts/fetch_today_orders.py)
- [backend/scripts/debug_email_4002217518.py](file://backend/scripts/debug_email_4002217518.py)
...
- [backend/src/直发API对接文档.md](file://backend/src/直发API对接文档.md)
...
- [backend/scripts/fix_email_logs_content_column.sql](file://backend/scripts/fix_email_logs_content_column.sql)
- [backend/scripts/update_email_logs_table.sql](file://backend/scripts/update_email_logs_table.sql)
</cite>

## 更新摘要
**变更内容**
- 新增客服外链系统API接口规范和数据结构定义
- 新增设计链接系统API接口规范和数据结构定义
- 新增客服外链数据库表结构和字段定义
- 新增客服外链前端组件和路由配置
- 新增设计链接前端组件和设计器集成
- 完善双门户架构下的外链访问控制机制
...
ETSY订单自动化管理系统是一个集成了前后端的完整订单处理解决方案。该系统通过FastAPI提供后端服务，Vue.js构建前端界面，实现从订单接收、处理到生产的全流程自动化管理。**最新更新**新增了客服外链系统和设计链接系统，支持独立的客服访问和设计修改功能，以及相关的数据库结构和前端组件。
...
- **翻译服务**: 集成智谱AI的中英文翻译服务，支持通用翻译和邮件专用翻译
- **文件管理**: SVG/PDF等文件的生成、存储和传输
- **实时监控**: 前端实时显示订单状态和处理进度
- **Supabase集成**: 完整的数据库操作和文件存储服务
- **双门户架构**: 支持商店门户、工厂门户和客服外链门户的独立访问
- **权限控制**: 多租户支持和子账号权限管理
- **物流集成**: 完整的4PX物流API集成，支持订单创建、面单获取、物流查询
- **邮件解析**: 增强的邮件解析功能，支持HTML格式和URL订单提取
- **字体管理**: 动态字体文件管理和服务，支持设计师工具
- **邮件管理**: 完整的邮件模板系统和邮件日志管理
- **客服外链**: 独立的客服访问入口，支持订单查看和邮件发送
- **设计链接**: 高级设计修改入口，支持实时设计器集成
...
系统采用前后端分离架构，后端提供RESTful API接口，前端通过Supabase进行数据库操作，同时调用后端API处理特殊功能。新增双门户架构支持商店、工厂和客服外链的独立访问，并集成了完整的物流服务、邮件解析、翻译服务、字体管理、客服外链和设计链接功能。
...
ShippingSvc[物流服务]
FontSvc[字体服务]
EmailParserSvc[邮件解析服务]
TranslationSvc[翻译服务]
ServiceLinkSvc[客服外链服务]
DesignLinkSvc[设计链接服务]
DB[数据库服务]
Config[配置管理]
FourPX[4PX API客户端]
Zhipu[智谱AI API]
...
FastAPI --> ShippingSvc
FastAPI --> FontSvc
FastAPI --> EmailParserSvc
FastAPI --> TranslationSvc
FastAPI --> ServiceLinkSvc
FastAPI --> DesignLinkSvc
ServiceLinkSvc --> TokenValidation
ServiceLinkSvc --> OrderAccess
ServiceLinkSvc --> OperationLogs
DesignLinkSvc --> TokenValidation
DesignLinkSvc --> OrderAccess
...
EmailSvc --> Templates
EmailSvc --> Logs
EmailParserSvc --> Parser
EmailParserSvc --> Extractor
TranslationSvc --> Zhipu
TranslationSvc --> TranslationEngine
TranslationSvc --> EmailTranslator
ShippingSvc --> FourPX
ShippingSvc --> Labels
...
Router --> ServicePortal
Router --> DesignPortal
Router --> AdminPortal
ServiceLink --> ServiceLinkSvc
DesignLink --> DesignLinkSvc
Designer --> FontSvc
OrdersPending --> Translation
OrdersPending --> Templates
OrdersPending --> Logs
EmailParser --> IMAP
EmailParser --> Parser
```
...
- [backend/src/api/service_link_routes.py:17-17](file://backend/src/api/service_link_routes.py#L17-L17)
- [backend/src/services/shipping_service.py:253-395](file://backend/src/services/shipping_service.py#L253-L395)
- [frontend/src/views/Admin/OrdersPending.vue:378-687](file://frontend/src/views/Admin/OrdersPending.vue#L378-L687)
...
| 邮件解析 | IMAPClient | - | 邮件服务器连接 |
| 翻译服务 | 智谱AI GLM-4 | - | 中英文翻译 |
| 邮件模板 | JSON配置 | - | 邮件内容模板 |
| 外链系统 | Token验证 | - | 客服访问控制 |
| 设计链接 | 独立设计器 | - | 设计修改工具 |
...
#### 客服外链门户 (Service Link Portal)
- **目标用户**: 客服人员
- **核心功能**: 订单查看、邮件发送、客户沟通
- **访问方式**: 通过 `/service/:shopCode?token=xxx` 路由访问
- **权限控制**: Token验证，订单访问限制

#### 设计链接门户 (Design Link Portal)
- **目标用户**: 客服人员（高级权限）
- **核心功能**: 设计修改、效果图生成、邮件回复
- **访问方式**: 通过 `/design/:shopCode?token=xxx` 路由访问
- **权限控制**: Token验证，设计权限控制
...
CheckPortal --> |客服外链| ServiceRoute["/service/:shopCode?token=*"]
CheckPortal --> |设计链接| DesignRoute["/design/:shopCode?token=*"]
CheckPortal --> |管理门户| AdminRoute["/admin/*"]
ShopRoute --> ShopAuth["商店认证检查"]
ServiceRoute --> ServiceAuth["Token验证"]
DesignRoute --> DesignAuth["Token验证"]
FactoryRoute --> FactoryFeatures["工厂功能"]
AdminRoute --> AdminAuth["管理员认证检查"]
ShopAuth --> AuthOK{"认证通过?"}
ServiceAuth --> TokenOK{"Token有效?"}
DesignAuth --> TokenOK
AuthOK --> |是| ShopFeatures["商店功能"]
TokenOK --> |是| ServiceFeatures["客服功能"]
TokenOK --> |是| DesignFeatures["设计功能"]
AuthOK --> |否| RedirectLogin["重定向到登录页"]
ServiceFeatures --> ServiceAccess["订单访问控制"]
DesignFeatures --> DesignAccess["设计权限控制"]
```

**图表来源**
- [frontend/src/router/index.js:67-80](file://frontend/src/router/index.js#L67-L80)
...
客服外链系统为客服人员提供独立的访问入口，支持订单查看、邮件发送和客户沟通功能。

### 客服外链架构

#### Token验证机制
- **Token生成**: 32字节随机十六进制字符串
- **Token存储**: shops表的service_token字段
- **Token验证**: 通过validate接口验证有效性
- **访问控制**: 仅允许启用的外链访问

#### 外链功能流程
```mermaid
flowchart TD
Start([客服访问外链]) --> ValidateToken["验证Token"]
ValidateToken --> TokenValid{"Token有效?"}
TokenValid --> |否| ShowError["显示错误页面"]
TokenValid --> |是| LoadOrders["加载待确认订单"]
LoadOrders --> ViewOrder["查看订单详情"]
ViewOrder --> SendEmail["发送邮件"]
SendEmail --> LogAction["记录操作日志"]
LogAction --> UpdateStatus["更新订单状态"]
UpdateStatus --> End([完成])
ShowError --> End
```

**图表来源**
- [backend/src/api/service_link_routes.py:208-246](file://backend/src/api/service_link_routes.py#L208-L246)
- [backend/src/api/service_link_routes.py:249-277](file://backend/src/api/service_link_routes.py#L249-L277)

### 客服外链API接口

#### 生成客服Token
- **方法**: POST
- **路径**: `/service-link/generate-token`
- **请求参数**:
  - `shop_id`: 店铺ID (UUID)
  - `enabled`: 是否启用 (布尔值，默认: true)

- **响应**: 包含完整的客服外链URL和Token信息

#### 获取客服Token
- **方法**: GET
- **路径**: `/service-link/shops/{shop_id}/token`
- **用途**: 获取店铺的客服外链信息

#### 切换外链状态
- **方法**: POST
- **路径**: `/service-link/shops/{shop_id}/toggle`
- **查询参数**: `enabled` (布尔值)
- **用途**: 启用或禁用客服外链

#### 验证客服Token
- **方法**: POST
- **路径**: `/service-link/validate`
- **请求参数**:
  - `shop_code`: 店铺代码
  - `token`: Token字符串
...
#### 获取待确认订单
- **方法**: GET
- **路径**: `/service-link/shops/{shop_id}/pending-orders`
- **用途**: 获取店铺的待确认订单列表

#### 记录操作日志
- **方法**: POST
- **路径**: `/service-link/log-action`
- **请求参数**:
  - `shop_id`: 店铺ID
  - `order_id`: 订单ID (可选)
  - `action`: 操作类型 (view/send_email/confirm/request_modify)
  - `action_details`: 操作详情 (JSON)
...
#### ServiceLink.vue 组件
- **功能**: 客服外链主界面
- **特性**: 订单列表、详情查看、邮件发送、操作历史
- **集成**: 与后端API的完整集成

#### Admin管理功能
- **生成外链**: 在店铺管理页面生成客服外链
- **复制外链**: 一键复制外链URL到剪贴板
- **刷新Token**: 刷新Token并更新外链
- **启用/禁用**: 控制外链的启用状态
- **查看日志**: 查看客服操作历史

**章节来源**
- [backend/src/api/service_link_routes.py:90-205](file://backend/src/api/service_link_routes.py#L90-L205)
- [backend/src/api/service_link_routes.py:208-329](file://backend/src/api/service_link_routes.py#L208-L329)
- [frontend/src/views/ServiceLink/ServiceLink.vue:1-818](file://frontend/src/views/ServiceLink/ServiceLink.vue#L1-L818)
- [frontend/src/views/Admin/AdminShops.vue:270-348](file://frontend/src/views/Admin/AdminShops.vue#L270-L348)
- [frontend/src/views/Admin/AdminUsers.vue:102-141](file://frontend/src/views/Admin/AdminUsers.vue#L102-L141)

## 设计链接系统

设计链接系统为客服人员提供高级设计修改功能，支持实时设计器集成和订单修改。

### 设计链接架构

#### 设计权限控制
- **Token生成**: 独立的设计链接Token
- **权限分离**: 与客服外链Token独立管理
- **设计器集成**: 通过iframe集成独立设计器
- **订单修改**: 支持实时设计修改和效果预览

#### 设计流程
```mermaid
flowchart TD
Start([客服访问设计链接]) --> ValidateDesignToken["验证设计链接Token"]
ValidateDesignToken --> TokenValid{"Token有效?"}
TokenValid --> |否| ShowError["显示错误页面"]
TokenValid --> |是| LoadOrders["加载待确认订单"]
LoadOrders --> SelectOrder["选择订单"]
SelectOrder --> LoadDesigner["加载设计器"]
LoadDesigner --> ModifyDesign["修改设计"]
ModifyDesign --> PreviewEffect["预览效果图"]
PreviewEffect --> SaveChanges["保存修改"]
SaveChanges --> LogAction["记录操作日志"]
LogAction --> UpdateStatus["更新订单状态"]
UpdateStatus --> End([完成])
ShowError --> End
```
...
**图表来源**
- [backend/src/api/service_link_routes.py:359-474](file://backend/src/api/service_link_routes.py#L359-L474)
- [frontend/src/views/DesignLink/DesignLink.vue:269-492](file://frontend/src/views/DesignLink/DesignLink.vue#L269-L492)

### 设计链接API接口

#### 生成设计链接Token
- **方法**: POST
- **路径**: `/service-link/design-link/generate-token`
- **请求参数**:
  - `shop_id`: 店铺ID (UUID)
  - `enabled`: 是否启用 (布尔值，默认: true)

- **响应**: 包含设计链接URL和Token信息

#### 获取设计链接Token
- **方法**: GET
- **路径**: `/service-link/design-link/shops/{shop_id}/token`
- **用途**: 获取店铺的设计链接信息

#### 切换设计链接状态
- **方法**: POST
- **路径**: `/service-link/design-link/shops/{shop_id}/toggle`
- **查询参数**: `enabled` (布尔值)
- **用途**: 启用或禁用设计链接

#### 验证设计链接Token
- **方法**: POST
- **路径**: `/service-link/design-link/validate`
- **请求参数**:
  - `shop_code`: 店铺代码
  - `token`: Token字符串
...
#### DesignLink.vue 组件
- **功能**: 设计链接主界面
- **特性**: 实时设计器、订单详情、邮件回复、历史记录
- **集成**: 与独立设计器的双向通信

#### 设计器通信机制
- **消息传递**: 通过postMessage与设计器通信
- **订单加载**: 自动加载订单数据到设计器
- **数据同步**: 实时同步设计器中的修改
- **回传处理**: 将设计器修改回传到系统

#### Admin管理功能
- **生成设计链接**: 在店铺管理页面生成设计链接
- **复制设计链接**: 一键复制设计链接URL到剪贴板
- **刷新设计Token**: 刷新设计链接Token
- **启用/禁用**: 控制设计链接的启用状态

**章节来源**
- [backend/src/api/service_link_routes.py:359-515](file://backend/src/api/service_link_routes.py#L359-L515)
- [frontend/src/views/DesignLink/DesignLink.vue:1-925](file://frontend/src/views/DesignLink/DesignLink.vue#L1-L925)
- [frontend/src/views/Admin/AdminShops.vue:270-348](file://frontend/src/views/Admin/AdminShops.vue#L270-L348)
- [frontend/src/views/Admin/AdminUsers.vue:143-200](file://frontend/src/views/Admin/AdminUsers.vue#L143-L200)

## API接口规范

系统提供以下核心API接口，支持双门户架构、权限控制和新增的客服外链系统：
...
#### 邮件专用翻译接口
- **方法**: POST
- **路径**: `/api/translate/email`
- **请求参数**:
  - `chinese_content`: 中文邮件内容 (字符串)
...
#### 解析订单邮件
- **方法**: POST
- **路径**: `/api/email/parse-order`
- **请求参数**:
  - `email_body`: 邮件正文内容
  - `email_subject`: 邮件主题
  - `email_from`: 发件人地址

- **响应**: 解析后的订单信息，支持HTML和纯文本格式

#### 提取URL订单
- **方法**: POST
- **路径**: `/api/email/extract-url-orders`
- **请求参数**:
  - `email_body`: 邮件正文内容
...
- [backend/src/api/service_link_routes.py:90-515](file://backend/src/api/service_link_routes.py#L90-L515)
- [backend/src/api/main.py:157-201](file://backend/src/api/main.py#L157-L201)
- [backend/src/api/main.py:796-883](file://backend/src/api/main.py#L796-L883)
- [docs/openapi.yaml:399-700](file://docs/openapi.yaml#L399-L700)

## 权限控制与多租户

系统实现了完整的多租户权限控制机制，支持主账号-子账号的层级权限管理，并新增了客服外链和设计链接的访问控制。
...
SERVICE_LINK_TOKENS ||--|| SHOPS : "belongs_to"
DESIGN_LINK_TOKENS ||--|| SHOPS : "belongs_to"
SERVICE_LINK_LOGS ||--|| SHOPS : "records"
```

**图表来源**
- [backend/scripts/init_multitenant.sql:95-116](file://backend/scripts/init_multitenant.sql#L95-L116)
- [backend/scripts/add_service_link_fields.sql:4-18](file://backend/scripts/add_service_link_fields.sql#L4-L18)
...
| **客服人员** | 客服权限 | 指定商店 | 客服外链访问、订单处理 |
| **设计人员** | 设计权限 | 指定商店 | 设计链接访问、设计修改 |
...
CheckRole --> |客服人员| CheckServiceToken["验证客服Token"]
CheckRole --> |设计人员| CheckDesignToken["验证设计Token"]
CheckShop --> HasPermission{"有权限?"}
CheckServiceToken --> ServiceTokenValid{"Token有效?"}
CheckDesignToken --> DesignTokenValid{"Token有效?"}
HasPermission --> |是| ShopAccess
ServiceTokenValid --> |是| ServiceAccess["客服功能访问"]
DesignTokenValid --> |是| DesignAccess["设计功能访问"]
HasPermission --> |否| AccessDenied["拒绝访问"]
ServiceTokenValid --> |否| AccessDenied
DesignTokenValid --> |否| AccessDenied
RequireAuth --> Login["登录页面"]
ServiceAccess --> Success["访问成功"]
DesignAccess --> Success
ShopAccess --> Success
...
3. **外链Token控制**: 客服外链和设计链接通过Token访问
4. **访问日志记录**: 记录所有用户的访问行为
5. **会话管理**: 24小时会话有效期
6. **订单访问限制**: 外链只能访问待确认订单
...
系统基于Supabase数据库，包含以下核心数据表，支持多租户架构和新增的客服外链系统：
...
| service_token | String(64) | 否 | NULL | 客服外链Token（新增） |
| service_link_enabled | Boolean | 否 | FALSE | 外链启用状态（新增） |
| service_link_created_at | Timestamp | 否 | NULL | 外链生成时间（新增） |
| service_link_updated_at | Timestamp | 否 | NULL | 外链更新时间（新增） |
| design_token | String(64) | 否 | NULL | 设计链接Token（新增） |
| design_link_enabled | Boolean | 否 | FALSE | 设计链接启用状态（新增） |
| design_link_created_at | Timestamp | 否 | NULL | 设计链接生成时间（新增） |
| design_link_updated_at | Timestamp | 否 | NULL | 设计链接更新时间（新增） |
| created_at | DateTime | 否 | NOW() | 创建时间 |
| updated_at | DateTime | 否 | NOW() | 更新时间 |
...
### 邮件解析表 (email_parsing_results)
**新增** 邮件解析结果表，支持订单信息提取

| 字段名 | 类型 | 必填 | 默认值 | 说明 |
|--------|------|------|--------|------|
| id | UUID | 是 | 自动生成 | 主键 |
| email_id | String(200) | 是 | - | 邮件唯一标识 |
| parsed_order_id | UUID | 否 | NULL | 关联解析后的订单 |
| parsing_result | JSON | 否 | NULL | 解析结果JSON |
| parsing_status | String(20) | 否 | 'pending' | 解析状态 |
| created_at | DateTime | 否 | NOW() | 创建时间 |
| updated_at | DateTime | 否 | NOW() | 更新时间 |

### 邮件日志表 (email_logs)
**新增** 邮件管理表，支持邮件内容和状态跟踪

| 字段名 | 类型 | 必填 | 默认值 | 说明 |
|--------|------|------|--------|------|
| id | UUID | 是 | 自动生成 | 主键 |
| order_id | Integer | 是 | - | 关联 orders.id |
| email_type | String(30) | 否 | NULL | 邮件类型（first_confirm/modification/follow_up） |
| recipient_email | String(100) | 否 | NULL | 收件人邮箱 |
| subject | String(200) | 否 | NULL | 邮件主题 |
| content | Text | 否 | NULL | 邮件内容（中英文对照） |
| sender_name | String(100) | 否 | 'Customer Support Team' | 落款人名称 |
| confirmation_deadline | String(100) | 否 | NULL | 确认截止时间 |
| status | String(20) | 否 | 'pending' | 发送状态 |
| sent_at | DateTime | 否 | NULL | 发送时间 |
| created_at | DateTime | 否 | NOW() | 创建时间 |
| updated_at | DateTime | 否 | NOW() | 更新时间 |

### 翻译记录表 (translation_logs)
**新增** 翻译服务使用记录表

| 字段名 | 类型 | 必填 | 默认值 | 说明 |
|--------|------|------|--------|------|
| id | UUID | 是 | 自动生成 | 主键 |
| request_type | String(20) | 否 | NULL | 请求类型（general/email） |
| source_lang | String(10) | 否 | NULL | 源语言 |
| target_lang | String(10) | 否 | NULL | 目标语言 |
| input_text | Text | 否 | NULL | 输入文本 |
| output_text | Text | 否 | NULL | 输出文本 |
| success | Boolean | 否 | NULL | 是否成功 |
| error_message | Text | 否 | NULL | 错误信息 |
| created_at | DateTime | 否 | NOW() | 创建时间 |

### 客服外链日志表 (service_link_logs)
**新增** 客服外链访问和操作日志表
...
| id | UUID | 是 | 自动生成 | 主键 |
| shop_id | UUID | 是 | - | 关联 shops.id |
| order_id | UUID | 否 | NULL | 关联 orders.id |
| ip_address | INET | 否 | NULL | 客服IP地址 |
| user_agent | TEXT | 否 | NULL | 客服用户代理 |
| action | String(50) | 是 | - | 操作类型（view/send_email/confirm/request_modify） |
| action_details | JSONB | 否 | NULL | 操作详情JSON |
...
- [backend/scripts/fix_email_logs_content_column.sql:1-15](file://backend/scripts/fix_email_logs_content_column.sql#L1-L15)
- [backend/scripts/update_email_logs_table.sql:1-21](file://backend/scripts/update_email_logs_table.sql#L1-L21)
- [backend/scripts/add_service_link_fields.sql:4-18](file://backend/scripts/add_service_link_fields.sql#L4-L18)
- [backend/scripts/create_service_link_logs_table.sql:4-31](file://backend/scripts/create_service_link_logs_table.sql#L4-L31)

## 前端状态管理

前端使用Pinia进行状态管理，提供完整的订单生命周期管理，支持双门户架构和新增的客服外链系统：
...
#### 邮件状态管理
- `emailContentChinese`: 中文邮件内容
- `emailContentEnglish`: 英文邮件内容
- `emailType`: 邮件类型（first_confirm/modification/follow_up）
- `selectedTemplate`: 选中的邮件模板
- `emailTone`: 邮件语气
- `emailLength`: 邮件长度
- `emailGreeting`: 称呼方式
- `senderName`: 落款人名称
- `customerNote`: 客户需求备注
- `isTranslating`: 翻译状态
- `confirmedEmailContent`: 已确认的邮件内容

#### 邮件操作功能
- `generateEmail()`: 生成邮件内容
- `translateEmail()`: 翻译邮件内容
- `copyEmail()`: 复制邮件内容
- `submitEmail()`: 提交邮件并保存到数据库
- `getEmailLogByOrderId(orderId)`: 获取订单的邮件日志

### 翻译服务状态管理
**新增** 翻译服务相关状态管理功能

#### 翻译状态
- `translateText`: 通用翻译文本
- `translateEmailContent`: 邮件翻译内容
- `isTranslating`: 翻译进行状态
- `translationError`: 翻译错误信息

#### 翻译操作
- `translateText()`: 调用通用翻译API
- `translateEmail()`: 调用邮件翻译API
- `handleTranslationError()`: 处理翻译错误

### 客服外链状态管理
**新增** 客服外链相关状态管理功能

#### 外链状态管理
- `serviceToken`: 客服外链Token
- `designToken`: 设计链接Token
- `serviceLinkEnabled`: 客服外链启用状态
- `designLinkEnabled`: 设计链接启用状态
- `pendingOrders`: 待确认订单列表
- `selectedOrder`: 当前选中订单
- `operationLogs`: 操作历史记录

#### 外链操作功能
- `generateServiceToken()`: 生成客服外链Token
- `generateDesignToken()`: 生成设计链接Token
- `validateToken()`: 验证Token有效性
- `fetchPendingOrders()`: 获取待确认订单
- `logAction()`: 记录操作日志
- `sendEmail()`: 发送邮件
- `confirmOrder()`: 确认订单
...
#### 设计操作功能
- `loadOrderToDesigner()`: 加载订单到设计器
- `saveDraft()`: 保存草稿
- `markAsProcessed()`: 标记已处理
- `onDesignerLoad()`: 设计器加载完成事件

**章节来源**
- [frontend/src/stores/orderStore.js:44-361](file://frontend/src/stores/orderStore.js#L44-L361)
- [frontend/src/stores/shopStore.js:9-190](file://frontend/src/stores/shopStore.js#L9-L190)
- [frontend/src/views/Admin/OrdersPending.vue:1040-1233](file://frontend/src/views/Admin/OrdersPending.vue#L1040-L1233)
...
系统实现了完整的文件生成和传输机制，支持双门户架构、客服外链系统和新增的设计链接功能：
...
participant EmailParser as 邮件解析服务
participant URLExtractor as URL提取器
participant DB as 数据库服务
EmailServer->>EmailParser : 接收订单邮件
EmailParser->>EmailParser : 解析HTML/纯文本格式
EmailParser->>EmailParser : 提取订单号和收件人信息
EmailParser->>URLExtractor : 分析邮件内容
URLExtractor->>URLExtractor : 提取URL中的订单号
URLExtractor-->>EmailParser : 返回URL订单号
EmailParser->>DB : 保存解析结果
DB-->>EmailParser : 返回订单ID
EmailParser-->>EmailServer : 返回解析完成
```

**图表来源**
- [backend/src/services/email_parser.py:61-294](file://backend/src/services/email_parser.py#L61-L294)
- [backend/src/services/email_service.py:87-200](file://backend/src/services/email_service.py#L87-L200)
...
participant TranslationSvc as 翻译服务
participant DB as 数据库服务
Client->>API : POST /api/translate/email
API->>TranslationSvc : translate_email()
TranslationSvc->>Zhipu : 调用翻译API
Zhipu-->>TranslationSvc : 返回翻译结果
TranslationSvc-->>API : 返回英文内容
API-->>Client : 返回翻译结果
Client->>API : POST /api/email/send-confirmation
API->>DB : 保存邮件日志
DB-->>API : 返回邮件ID
API-->>Client : 返回发送结果
```
...
**图表来源**
- [backend/src/api/main.py:844-883](file://backend/src/api/main.py#L844-L883)
- [backend/src/services/translation_service.py:87-155](file://backend/src/services/translation_service.py#L87-L155)
- [backend/src/services/email_service.py:174-247](file://backend/src/services/email_service.py#L174-L247)

### 字体加载和设计器通信流程
**新增** 前端设计器与后端字体服务的通信流程

```mermaid
sequenceDiagram
participant Designer as 独立设计器
participant FontAPI as 字体API
participant Backend as 后端服务
Designer->>FontAPI : 请求字体文件
FontAPI->>Backend : 获取字体数据
Backend-->>FontAPI : 返回字体文件
FontAPI-->>Designer : 返回字体内容
Designer->>Designer : 加载字体到设计器
Designer->>Backend : 发送订单数据
Backend-->>Designer : 返回订单确认
```

**图表来源**
- [frontend/public/designer-standalone.html:986-1035](file://frontend/public/designer-standalone.html#L986-L1035)
- [backend/src/api/main.py:157-201](file://backend/src/api/main.py#L157-L201)
...
```mermaid
sequenceDiagram
participant ServiceUser as 客服用户
participant ServicePortal as 客服外链门户
participant ServiceAPI as 客服外链API
participant DB as 数据库服务
ServiceUser->>ServicePortal : 访问客服外链
ServicePortal->>ServiceAPI : 验证Token
ServiceAPI->>DB : 检查Token有效性
DB-->>ServiceAPI : 返回Token状态
ServiceAPI-->>ServicePortal : 返回验证结果
ServicePortal->>ServiceAPI : 获取待确认订单
ServiceAPI->>DB : 查询订单数据
DB-->>ServiceAPI : 返回订单列表
ServiceAPI-->>ServicePortal : 返回订单数据
ServicePortal->>ServiceAPI : 发送邮件
ServiceAPI->>DB : 记录操作日志
DB-->>ServiceAPI : 返回日志ID
ServiceAPI-->>ServicePortal : 返回操作结果
```

**图表来源**
- [backend/src/api/service_link_routes.py:208-246](file://backend/src/api/service_link_routes.py#L208-L246)
- [backend/src/api/service_link_routes.py:249-329](file://backend/src/api/service_link_routes.py#L249-L329)
...
```mermaid
sequenceDiagram
participant DesignUser as 设计人员
participant DesignPortal as 设计链接门户
participant DesignAPI as 设计链接API
participant Designer as 独立设计器
DesignUser->>DesignPortal : 访问设计链接
DesignPortal->>DesignAPI : 验证设计Token
DesignAPI->>DB : 检查设计Token
DB-->>DesignAPI : 返回Token状态
DesignAPI-->>DesignPortal : 返回验证结果
DesignPortal->>DesignAPI : 获取待确认订单
DesignAPI->>DB : 查询订单数据
DB-->>DesignAPI : 返回订单列表
DesignAPI-->>DesignPortal : 返回订单数据
DesignPortal->>Designer : 加载设计器
Designer->>DesignPortal : 加载订单数据
DesignPortal->>Designer : 发送设计修改
Designer->>DesignPortal : 返回修改结果
DesignPortal->>DesignAPI : 记录操作日志
DesignAPI->>DB : 保存日志
DB-->>DesignAPI : 返回日志ID
DesignAPI-->>DesignPortal : 返回操作结果
```
...
- [backend/src/services/email_parser.py:61-294](file://backend/src/services/email_parser.py#L61-L294)
- [backend/src/services/translation_service.py:21-85](file://backend/src/services/translation_service.py#L21-L85)
- [backend/src/api/service_link_routes.py:90-515](file://backend/src/api/service_link_routes.py#L90-L515)
...
系统增强了邮件解析功能，支持HTML地址格式和URL订单提取，提高了订单自动化的准确性。

### 邮件解析器架构

#### 多格式支持
- **HTML格式解析**: 支持带有HTML标签的邮件格式
- **纯文本格式解析**: 支持传统的纯文本邮件格式
- **URL订单提取**: 自动从邮件URL中提取订单号
- **转发邮件处理**: 支持转发邮件的嵌套内容解析

#### 解析流程
```mermaid
flowchart TD
Start([接收邮件]) --> DetectFormat{"检测邮件格式"}
DetectFormat --> |HTML| ParseHTML["HTML格式解析"]
DetectFormat --> |纯文本| ParseText["纯文本格式解析"]
DetectFormat --> |转发邮件| ParseForwarded["转发邮件解析"]
ParseHTML --> ExtractOrder["提取订单信息"]
ParseText --> ExtractOrder
ParseForwarded --> ExtractOrder
ExtractOrder --> ExtractURL["提取URL订单"]
ExtractURL --> Validate["验证订单信息"]
Validate --> SaveResult["保存解析结果"]
SaveResult --> End([完成])
```
...
<address>
  ...
  <span class='city'>EMU PLAINS</span> 
  <span class='state'>NSW</span> 
  <span class='zip'>2750</span>
  <span class='country-name'>Australia</span>
</address>
`;

// 提取各个字段
const name = extractSpanContent(htmlAddress, 'name'); // Jessica Head
const address = extractSpanContent(htmlAddress, 'first-line'); // 26 Yodalla Ave
const city = extractSpanContent(htmlAddress, 'city'); // EMU PLAINS
const state = extractSpanContent(htmlAddress, 'state'); // NSW
const zip = extractSpanContent(htmlAddress, 'zip'); // 2750
const country = extractSpanContent(htmlAddress, 'country-name'); // Australia
```

#### 地址字段提取规则
- **name**: 收件人姓名，提取span class为'name'的内容
- **first-line**: 地址第一行，提取span class为'first-line'的内容
- **city**: 城市，提取span class为'city'的内容
- **state**: 州/省，提取span class为'state'的内容
- **zip**: 邮政编码，提取span class为'zip'的内容
- **country-name**: 国家名称，提取span class为'country-name'的内容

### URL订单提取功能

#### URL模式识别
系统能够从邮件内容中自动识别和提取Etsy订单URL：

```javascript
// 支持的URL模式
const urlPatterns = [
  /etsy\.com\/your\/orders\/(\d+)/,  // 主要订单URL格式
  /Order\s*#(\d+)/,                   // 订单号直接格式
  /Your order number is:?\s*(\d+)/,   // 订单号说明格式
];

// 从邮件内容提取订单号
function extractOrderNumbers(emailBody) {
  const orderNumbers = [];
  urlPatterns.forEach(pattern => {
    const matches = emailBody.match(pattern);
    if (matches) {
      orderNumbers.push(...matches.slice(1));
    }
  })
  return [...new Set(orderNumbers)]; // 去重
}
```

#### URL提取流程
1. **模式匹配**: 使用正则表达式匹配各种URL格式
2. **订单号提取**: 从匹配结果中提取订单号
3. **去重处理**: 移除重复的订单号
4. **验证检查**: 验证提取的订单号格式
...
def search_all_unread_etsy_orders(self) -> List[int]:
    ...
    """
    # 优先搜索直接来自 transaction@etsy.com 的订单邮件
...
- [backend/src/services/email_parser.py:61-294](file://backend/src/services/email_parser.py#L61-L294)
- [backend/src/services/email_service.py:87-200](file://backend/src/services/email_service.py#L87-L200)
- [backend/scripts/fetch_today_orders.py:111-139](file://backend/scripts/fetch_today_orders.py#L111-L139)
- [backend/scripts/debug_email_4002217518.py:44-59](file://backend/scripts/debug_email_4002217518.py#L44-L59)

## 翻译服务集成

系统集成了智谱AI的翻译服务，提供通用翻译和邮件专用翻译功能，支持中英文互译。

### 翻译服务架构

#### 智谱AI集成
- **API服务**: 使用GLM-4模型进行高质量翻译
- **API密钥管理**: 通过环境变量配置
- **请求超时**: 30秒超时设置
- **错误处理**: 完善的异常捕获和错误返回
...
#### 通用翻译流程
1. **参数验证**: 检查输入文本和语言参数
2. **系统提示构建**: 根据源语言和目标语言构建优化提示
3. **API调用**: 调用智谱AI翻译API
4. **结果处理**: 解析API响应，提取翻译结果
5. **错误处理**: 处理API错误和异常情况

#### 邮件翻译流程
1. **邮件专用提示**: 使用专业的邮件翻译提示词
2. **格式保持**: 维持邮件格式、换行和特殊字符
3. **语气适配**: 保持客户服务的专业语气
4. **表情符号处理**: 保留表情符号和特殊字符
...
- **请求参数**:
  - `text`: 要翻译的文本
  - `source_lang`: 源语言 (zh/en，默认: zh)
  - `target_lang`: 目标语言 (zh/en，默认: en)
...
- [backend/src/api/main.py:782-883](file://backend/src/api/main.py#L782-L883)
- [frontend/src/views/Admin/OrdersPending.vue:1050-1083](file://frontend/src/views/Admin/OrdersPending.vue#L1050-L1083)
...
window.addEventListener('message', function(event) {
  if (event.data && event.data.type === 'loadOrder') {
    const data = event.data.data
    console.log('📥 设计器收到订单数据:', data)
    
    // 更新表单字段（使用正确的ID）
    if (data.frontText !== undefined) {
      const el = document.getElementById('ed-fc')
      if (el) el.value = data.frontText
    }
    if (data.backText !== undefined) {
      const el = document.getElementById('ed-bc')
      if (el) el.value = data.backText
    }
    if (data.phone !== undefined) {
      const el = document.getElementById('ed-phone')
      if (el) el.value = data.phone
    }
    if (data.shape) {
      const el = document.getElementById('ed-shape')
      if (el) el.value = data.shape
    }
    if (data.color) {
      const el = document.getElementById('ed-color')
      if (el) el.value = data.color
    }
    if (data.font) {
      const el1 = document.getElementById('ed-ff')
      const el2 = document.getElementById('ed-bf')
      if (el1) el1.value = data.font
      if (el2) el2.value = data.font
    }
    ...
    // 通知父页面加载完成
    event.source.postMessage({ type: 'designerLoaded', orderId: data.orderId }, '*')
  }
})
```
...
系统实现了多层次的错误处理机制，支持双门户架构、客服外链系统和新增的设计链接功能：
...
- **邮件解析错误**: 邮件格式不支持和解析失败处理
- **翻译服务错误**: 智谱AI API错误和翻译失败处理
- **邮件日志错误**: 邮件内容保存和状态更新错误
- **外链Token错误**: Token验证失败和权限不足错误
- **设计链接错误**: 设计权限不足和设计器通信错误
...
- **邮件解析错误**: 邮件格式不支持的错误处理
- **翻译服务错误**: 翻译API调用失败的错误处理
- **邮件模板错误**: 邮件模板加载和渲染错误
- **外链访问错误**: Token无效和权限不足的错误处理
- **设计器通信错误**: 设计器加载失败和通信异常处理
...
- **字体目录验证**: 确保字体文件存在且可访问
- **邮件服务器验证**: 验证IMAP服务器配置
- **翻译API验证**: 验证智谱AI API密钥配置
- **外链配置验证**: 验证外链Token和权限配置
...
#### 客服外链门户错误
- **Token无效**: 显示"客服外链Token无效或已过期"
- **外链禁用**: 显示"客服外链已被禁用"
- **权限不足**: 显示"您没有访问该商店客服外链的权限"

#### 设计链接门户错误
- **设计权限不足**: 显示"您没有访问设计链接的权限"
- **设计器加载失败**: 显示"设计器加载失败，请重试"
- **订单访问限制**: 显示"无法访问该订单的设计权限"
...
#### 邮件格式错误
- **HTML解析失败**: 显示"邮件HTML格式解析失败"
- **纯文本解析失败**: 显示"邮件纯文本解析失败"
- **URL提取失败**: 显示"URL订单提取失败"

#### 订单信息缺失
- **订单号缺失**: 显示"邮件中未找到订单号"
- **收件人信息缺失**: 显示"邮件中未找到收件人信息"
- **地址格式错误**: 显示"收件人地址格式不正确"

### 翻译服务错误处理

#### 翻译API错误
- **API密钥缺失**: 显示"翻译API密钥未配置"
- **请求超时**: 显示"翻译API请求超时"
- **翻译失败**: 显示"翻译服务异常"
- **内容为空**: 显示"翻译内容不能为空"
...
#### Token验证错误
- **Token不存在**: 显示"Token不存在"
- **Token不匹配**: 显示"Token不匹配"
- **Token已过期**: 显示"Token已过期"
- **外链未启用**: 显示"外链未启用"

#### 订单访问错误
- **订单不属于该商店**: 显示"无法访问该订单"
- **订单状态不允许**: 显示"该订单状态不允许访问"
- **权限不足**: 显示"您没有访问该订单的权限"
...
- [backend/src/services/email_parser.py:61-294](file://backend/src/services/email_parser.py#L61-L294)
- [backend/src/services/translation_service.py:33-35](file://backend/src/services/translation_service.py#L33-L35)
- [backend/src/api/service_link_routes.py:208-246](file://backend/src/api/service_link_routes.py#L208-L246)
...
```sql
-- 更新邮件日志表结构
-- 参考 backend/scripts/update_email_logs_table.sql

-- 修复邮件日志表缺失字段
-- 参考 backend/scripts/fix_email_logs_content_column.sql
```

#### 客服外链数据库初始化
```sql
-- 执行客服外链字段SQL脚本
# 参考 backend/scripts/add_service_link_fields.sql

-- 创建客服外链日志表
# 参考 backend/scripts/create_service_link_logs_table.sql

-- 执行数据库初始化脚本
python backend/scripts/setup_service_link.py
```
...
#### 智谱AI配置
- **API密钥**: 在`.env`文件中配置`ZHIPU_API_KEY`
- **模型选择**: 通过`ZHIPU_MODEL`配置翻译模型
- **超时设置**: 默认30秒超时
- **温度参数**: 默认0.3，保证翻译稳定性
...
#### IMAP配置
- **服务器设置**: 配置IMAP_SERVER和IMAP_PORT
- **认证设置**: 配置EMAIL_ADDRESS和EMAIL_PASSWORD
- **搜索范围**: 设置邮件搜索的时间范围和过滤条件
...
#### 客服外链配置
- **Token生成**: 使用32字节随机十六进制字符串
- **Token存储**: shops表的service_token字段
- **外链启用**: 通过service_link_enabled字段控制
- **访问限制**: 仅允许待确认订单访问

#### 设计链接配置
- **设计Token**: 独立的设计链接Token
- **权限分离**: 与客服外链Token独立管理
- **设计器集成**: 通过iframe集成独立设计器
- **操作日志**: 记录设计修改操作
...
- [backend/scripts/update_email_logs_table.sql:1-21](file://backend/scripts/update_email_logs_table.sql#L1-L21)
- [backend/scripts/fix_email_logs_content_column.sql:1-15](file://backend/scripts/fix_email_logs_content_column.sql#L1-L15)
- [backend/scripts/add_service_link_fields.sql:1-32](file://backend/scripts/add_service_link_fields.sql#L1-L32)
- [backend/scripts/create_service_link_logs_table.sql:1-38](file://backend/scripts/create_service_link_logs_table.sql#L1-L38)
- [backend/scripts/setup_service_link.py:1-92](file://backend/scripts/setup_service_link.py#L1-L92)
...
# 邮件解析相关请求模型
class EmailParseOrderRequest(BaseModel):
    email_body: str
    email_subject: str
    email_from: str

class EmailExtractUrlOrdersRequest(BaseModel):
    email_body: str
...
// 翻译服务API
export async function translateText(text, sourceLang = 'zh', targetLang = 'en') {
...
export async function translateEmail(chineseContent) {
  const response = await fetch(`${API_BASE_URL}/api/translate/email`, {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({ chinese_content: chineseContent }),
  })
  
  if (!response.ok) {
    throw new Error('邮件翻译失败')
  }
...
// 邮件解析API
export async function parseOrderEmail(emailData) {
  const response = await fetch(`${API_BASE_URL}/api/email/parse-order`, {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
  ...
  if (!response.ok) {
    const error = await response.json()
    throw new Error(error.message || '邮件解析失败')
  }
  
  return response.json()
}

export async function extractUrlOrders(emailBody) {
  const response = await fetch(`${API_BASE_URL}/api/email/extract-url-orders`, {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
  ...
  if (!response.ok) {
    const error = await response.json()
    throw new Error(error.message || 'URL订单提取失败')
  }
...
// 客服外链API
export async function generateServiceToken(shopId, enabled = true) {
  const response = await fetch(`${API_BASE_URL}/service-link/generate-token`, {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({ shop_id: shopId, enabled }),
  })
  
  if (!response.ok) {
    throw new Error('生成客服Token失败')
  }
  
  return response.json()
}

export async function validateServiceToken(shopCode, token) {
  const response = await fetch(`${API_BASE_URL}/service-link/validate`, {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({ shop_code: shopCode, token }),
  })
...
export async function fetchPendingOrders(shopId) {
  const response = await fetch(`${API_BASE_URL}/service-link/shops/${shopId}/pending-orders`)
...
export async function logServiceAction(shopId, orderId, action, details) {
  const response = await fetch(`${API_BASE_URL}/service-link/log-action`, {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({ shop_id: shopId, order_id: orderId, action, action_details: details }),
  })
...
// 设计链接API
export async function generateDesignToken(shopId, enabled = true) {
  const response = await fetch(`${API_BASE_URL}/service-link/design-link/generate-token`, {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({ shop_id: shopId, enabled }),
  })
  ...
  if (!response.ok) {
    throw new Error('生成设计Token失败')
  }
  
  return response.json()
}

export async function validateDesignToken(shopCode, token) {
  const response = await fetch(`${API_BASE_URL}/service-link/design-link/validate`, {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({ shop_code: shopCode, token }),
  })
...
const { data } = await supabase
  .from('shops')
  .select('service_token, service_link_enabled')
  .eq('id', shopId)

// 设计链接Token查询
const { data } = await supabase
  .from('shops')
  .select('design_token, design_link_enabled')
  .eq('id', shopId)
...
# 测试邮件解析
python backend/scripts/debug_email_4002217518.py

# 测试翻译服务
python backend/scripts/test_translation_service.py

# 测试客服外链
python backend/scripts/test_service_link_api.py

# 测试设计链接
python backend/scripts/test_design_link_api.py
```
...
# 测试邮件翻译
email_result = translator.translate_email("亲爱的客户，感谢您的订单")
print(email_result)
```
...
```javascript
// 测试邮件模板加载
const templates = await fetch('/src/config/email-templates.json')
const templateData = await templates.json()

// 测试邮件内容生成
const emailContent = generateEmailContent(
  templateData.first_confirm.templates.standard.content.formal.standard
)
...
```python
# 测试邮件解析功能
from backend.src.services.email_parser import EtsyEmailParser
...
Delivery Address:
<address>
  <span class='name'>Test Customer</span>
  <span class='first-line'>123 Test St</span>
  <span class='city'>Test City</span> 
  <span class='state'>Test State</span> 
  <span class='zip'>12345</span>
  <span class='country-name'>Test Country</span>
</address>
"""

parsed = parser.parse(html_email)
print(parsed.shipping_name)  # Test Customer
print(parsed.shipping_address_line1)  # 123 Test St
```
...
```python
# 测试客服外链功能
from backend.src.api.service_link_routes import generate_secure_token
...
# 测试URL构建
from backend.src.api.service_link_routes import build_service_link_url
url = build_service_link_url("US001", token)
print(f"Service link URL: {url}")
```
...
# 测试设计Token生成
design_token = generate_secure_token()
print(f"Generated design token: {design_token}")

# 测试URL构建
from backend.src.api.service_link_routes import build_design_link_url
design_url = build_design_link_url("US001", design_token)
print(f"Design link URL: {design_url}")
```
...
- [backend/src/services/email_parser.py:61-294](file://backend/src/services/email_parser.py#L61-L294)
- [backend/src/services/translation_service.py:21-85](file://backend/src/services/translation_service.py#L21-L85)
- [backend/scripts/test_service_link_api.py:1-50](file://backend/scripts/test_service_link_api.py#L1-L50)
- [backend/scripts/test_design_link_api.py:1-50](file://backend/scripts/test_design_link_api.py#L1-L50)
...
- `email-parsing-results`: 邮件解析结果存储
- `email-logs`: 邮件日志存储
- `translation-logs`: 翻译记录存储
- `service-link-logs`: 客服外链日志存储
...
EMAIL_PARSING_RESULTS ||--|| ORDERS : "parses_to"
USERS ||--o{ USER_SHOP_PERMISSIONS : "has"
USER_SHOP_PERMISSIONS ||--|| SHOPS : "grants"
SHOP_ACCESS_LOGS ||--|| SHOPS : "records"
FACTORY_ACCESS_LOGS ||--|| USERS : "records"
LOGISTICS ||--|| ORDERS : "belongs_to"
EMAIL_LOGS ||--|| ORDERS : "belongs_to"
TRANSLATION_LOGS ||--|| ORDERS : "belongs_to"
SERVICE_LINK_LOGS ||--|| SHOPS : "records"
SERVICE_LINK_LOGS ||--|| ORDERS : "logs"
SHOPS ||--o{ SERVICE_LINK_LOGS : "has"
SHOPS ||--o{ DESIGN_LINK_LOGS : "has"
```
