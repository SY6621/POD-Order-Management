...
## 更新摘要
**变更内容**
- 新增翻译服务API端点（/api/translate, /api/translate/email）
- 增强邮件管理功能，支持中英文邮件内容生成和管理
- 优化OrdersPending标签页UI，集成翻译功能和邮件模板系统
- 完善邮件日志管理，支持落款人和确认截止时间字段
- 集成智谱AI翻译服务，支持通用翻译和邮件专用翻译
...
ETSY订单自动化管理系统是一个集成了前后端的完整订单处理解决方案。该系统通过FastAPI提供后端服务，Vue.js构建前端界面，实现从订单接收、处理到生产的全流程自动化管理。**最新更新**新增了翻译服务API端点（/api/translate, /api/translate/email）和邮件管理相关接口，以及OrdersPending标签页UI优化的前端后端对接。
...
系统采用前后端分离架构，后端提供RESTful API接口，前端通过Supabase进行数据库操作，同时调用后端API处理特殊功能。新增双门户架构支持商店和工厂的独立访问，并集成了完整的物流服务、邮件解析、翻译服务和字体管理功能。
...
系统提供以下核心API接口，支持双门户架构、权限控制和新增的翻译服务：
...
系统实现了完整的文件生成和传输机制，支持双门户架构和新增的翻译服务：
...
系统实现了多层次的错误处理机制，支持双门户架构和新增的翻译服务：
